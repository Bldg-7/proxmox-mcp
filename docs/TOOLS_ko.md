...
| 엔드포인트 | 메서드 | 설명 |
|-----------|--------|------|
| `/nodes/{node}/services` | GET | 노드 서비스 목록 |
| `/nodes/{node}/services/{service}` | POST | 서비스 시작/중지/재시작 |
| `/nodes/{node}/syslog` | GET | 시스템 로그 읽기 |
| `/nodes/{node}/journal` | GET | systemd 저널 읽기 |
| `/nodes/{node}/tasks` | GET | 노드 작업 목록 |
| `/nodes/{node}/tasks/{upid}` | GET | 작업 상태/로그 조회 |
| `/nodes/{node}/aplinfo` | GET | 사용 가능한 어플라이언스 템플릿 목록 |
| `/nodes/{node}/netstat` | GET | 네트워크 통계 |
