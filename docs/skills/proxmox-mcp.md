...
> AI-optimized reference for using the Proxmox MCP server to manage Proxmox Virtual Environment

**Generated**: 2026-02-06  
**Total Tools**: 227  
**MCP Server**: `@bldg-7/proxmox-mcp`

---

## MCP Server Information

This MCP server provides 227 tools for comprehensive Proxmox Virtual Environment management through the Model Context Protocol. It enables AI agents to:

- Manage QEMU VMs and LXC containers (create, configure, lifecycle operations)
- Control cluster resources (HA, replication, backup jobs)
- Manage storage, networking, and access control
- Monitor nodes, services, and tasks
- Execute commands via QEMU guest agent
- Manage Ceph storage and SDN (Software Defined Networking)

**Installation**: `npm install @bldg-7/proxmox-mcp`

**Usage**: Configure in Claude Desktop or any MCP-compatible client with environment variables.
...
|----------|-------------|---------|
| `PROXMOX_USER` | Username with realm (e.g., `root@pam`) | `root@pam` |
| `PROXMOX_SSL_MODE` | SSL verification mode: `strict`, `verify`, `insecure` | `strict` |
| `PROXMOX_ALLOW_ELEVATED` | Allow elevated operations (create/modify/delete) | `false` |
...
- **86 basic tools**: Query operations (list, get, status)
- **141 elevated tools**: Destructive operations (create, update, delete, control)

**Safety**: Elevated operations require explicit opt-in via environment variable to prevent accidental destructive actions.
